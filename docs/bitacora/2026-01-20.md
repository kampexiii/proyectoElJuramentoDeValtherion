...
- **Diseño No-Scroll**: Estructura fija de `100dvh` (Dynamic Viewport Height) con un contenedor central `flex: 1` y `overflow: hidden`.
- **Responsividad Total**: Se ajustaron todas las pantallas placeholder y el home para que su contenido se adapte al espacio entre navbars sin generar scroll en móvil ni PC, utilizando clases de Flexbox y truncado de texto.
- **Doble Navbar**:
...
- **Simplificación UI**: Se eliminó la sección de "Acciones Rápidas" y las estadísticas (oro, nivel, etc.) del Home para usuarios sin personaje, mostrando en su lugar información de la cuenta y un botón para crear personaje.
- **Marca**: Se aumentó el tamaño del logo en la navbar superior para mayor visibilidad.

## Verificación

- **Navegador**: Verificado que `/home` muestra el estado de "Aún no has forjado tu leyenda" y los datos de cuenta. El logo se ve notablemente más grande y centrado.
