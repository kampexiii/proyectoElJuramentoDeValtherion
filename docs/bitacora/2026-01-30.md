---
## Refactor final: stats 100% dinámicas y equipamiento flexible (30/01/2026)

### Archivos tocados:
- app/Models/Character.php
- app/Services/StatCalculatorService.php (nuevo)
...
### Cambios principales:
- Se eliminó cualquier lógica de montura fija/admin: ahora todas las monturas y equipos pueden equiparse/desequiparse libremente.
- Se centralizó el cálculo de stats efectivos en StatCalculatorService, usado por Character::effectiveStats().
- Todos los lugares donde se muestran stats (perfil, home, equipamiento) usan ahora stats calculados dinámicamente.
- La UI de equipamiento permite siempre cambiar montura y equipo, sin restricciones ocultas.
- No se guarda nunca stats en la BD al equipar/desequipar: siempre se calcula en tiempo real.
- Confirmado: no se ha tocado ni subido .env ni database/db_proyecto_sevillano.sqlite.

### Verificación:
- Stats mostradas en home y perfil cambian dinámicamente al cambiar equipamiento/montura.
- No existe lógica de montura fija ni para admin ni para ningún usuario.
---

---

## Fix montura no fija admin + max stats (30/01/2026)
...
La montura especial ya no se asigna automáticamente al admin ni a la raza especial. Ahora existe solo como una montura más, equipable por cualquier personaje.

### Funcionamiento del efecto “max stats”:

Al equipar la montura especial (code: mount_legendario_caos), el personaje obtiene automáticamente los stats máximos de su raza (según caps_json). El efecto solo se aplica mientras la montura está equipada. Si no hay caps_json, se usan los stats actuales como fallback, sin romper la app.

### Confirmación:

- No se ha tocado ni subido .env ni database/db_proyecto_sevillano.sqlite
...
No se tocó .env ni db_proyecto_sevillano.sqlite.
...
**Ajustes adicionales (sprites y UI):**

- Agregada columna 'sprite' a tabla 'races' con migración.
- Actualizado RaceSeeder para incluir sprites, especialmente 'Aldrik.png' para raza admin.
- Sprite del personaje mostrado en home y edit, con fallback a sprite de raza.
- Barra de HP agregada en edit con valores actuales/máximos.
- Sprite oculto en mobile en edit para evitar overflow.
- Ajustes de altura en edit para mobile (auto height).
- Secciones "Inventario rápido" y "Equipar objeto" ocultas en mobile.
- Sprite triplicado en desktop (384x384px).

Archivos tocados adicionales:

- database/migrations/2026_01_30_000031_add_sprite_to_races_table.php
- public/assets/sprites/razas/Aldrik.png
- app/Http/Controllers/HomeController.php (sprite logic)
- resources/css/game/components/home_index.css (sprite styles y responsive)
