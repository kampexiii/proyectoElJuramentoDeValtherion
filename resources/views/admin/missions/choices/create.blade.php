    ...
    <div class="row g-3 h-100">
        <div class="col-12">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-2">
                <div>
                    <h1 class="h5 mb-0">Crear opcion</h1>
        ...
        <div class="col-12">
            <form method="POST" action="{{ route('missions.nodes.choices.store', [$mission, $node]) }}" class="d-grid gap-3">
                @csrf
                <div class="card bg-zinc-900 border-secondary text-white shadow-sm">
                    <div class="card-body p-3 d-grid gap-2">
                        <div>
                            ...
                            <input id="choice_text" name="choice_text" class="form-control form-control-sm" value="{{ old('choice_text') }}" required>
                        </div>

                        <div>
                            <label class="form-label" for="outcome_text">Texto de resultado</label>
                            <textarea id="outcome_text" name="outcome_text" rows="3" class="form-control form-control-sm">{{ old('outcome_text') }}</textarea>
                        </div>
                        ...
                        <div class="row g-2">
                            <div class="col-12 col-md-4">
                                ...
                                <select id="difficulty_points" name="difficulty_points" class="form-select form-select-sm" required>
                                    @for ($i = 0; $i <= 3; $i++)
                                        <option value="{{ $i }}" @selected((int) old('difficulty_points', 0) === $i)>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="col-12 col-md-4">
                                ...
                                <select id="choice_order" name="order" class="form-select form-select-sm" required>
                                    @foreach ($orderOptions as $order)
                                        <option value="{{ $order }}" @selected((int) old('order', 1) === $order)>{{ $order }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <select id="next_node_id" name="next_node_id" class="form-select form-select-sm" required>
                                    ...
                                    @foreach ($nextNodes as $nextNode)
                                        <option value="{{ $nextNode->id }}" @selected((int) old('next_node_id') === $nextNode->id)>
                        ...
                        <div>
                            ...
                            <textarea id="effects_json_raw" name="effects_json_raw" rows="4" class="form-control form-control-sm" placeholder='{"wounds":1}'>{{ old('effects_json_raw') }}</textarea>
                        </div>
                    </div>
                </div>
                <button type="submit" class="btn btn-outline-info btn-sm">Crear opcion</button>
            </form>
        </div>
    </div>
</div>
